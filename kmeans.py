...
def euclideanDistance(pointA, pointB):
    ...
    ''''blue = math.pow(int(pointA[0]) - int(pointB[0]), 2)
    red = math.pow(int(pointA[1]) - int(pointB[1]), 2)
    green = math.pow(int(pointA[2]) - int(pointB[2]), 2)'''
...
def centroidsRandomGeneration(img, kCentroids, clustersCentroids, clustersPoints, rows, cols):
    for i in range(kCentroids):
        ...
        '''centroidScalarValues = [0] * d
        for k in range(d):
            centroidScalarValues[k] = img[centroidY][centroidX][k]'''
...
def adjustCentroidsPositions(img, clustersCentroids, clustersPoints):
    ...
    for i in range(nClusters):
        ...
        for point in clustersPoints[i]:
            channelsMean = [channelsMean[k] + img[point[0]][point[1]][k] for k in range(d)]
        oldCentroidValues = np.copy(centroid)
        if nPoints > 0:
            for k in range(d):
                centroid[k] = channelsMean[k] / nPoints
...
def applyFinalClusterToImage(img, clustersCentroids, clustersPoints):
    ...
    for i in range(len(clustersCentroids)):
        ...
        for point in clustersPoints[i]:
            for k in range(d):
                dst[point[0]][point[1]][k] = centroid[k]
    return dst


def kmeans(src, kCentroids):
